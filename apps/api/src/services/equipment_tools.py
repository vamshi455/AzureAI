"""
Equipment Health Agent Tools - PostgreSQL and pgvector query implementations.
...
class EquipmentTools:
    """Tool implementations for the Equipment Health Agent."""
    ...
    async def list_equipment_by_risk(
        self, risk_level: str, plant_code: str | None = None, limit: int = 20
    ) -> dict:
        ...
        query = """
            SELECT equipment_id, plant_code, production_line, health_score,
                   ...
                   recommended_action
            FROM equipment_health
            WHERE failure_risk = $1
        """
        params: list = [risk_level]

        if plant_code:
            query += " AND plant_code = $2"
            params.append(plant_code)

        query += " ORDER BY failure_probability DESC LIMIT $" + str(len(params) + 1)
        params.append(limit)
    ...
    async def get_sensor_readings(
        ...
        query = """
            ...
            FROM iot_telemetry
            WHERE equipment_id = $1 AND reading_timestamp >= $2
        """
        ...
        query += " ORDER BY reading_timestamp DESC LIMIT 200"
    ...
    async def get_equipment_anomalies(
        ...
        query += " ORDER BY reading_timestamp DESC LIMIT 100"
    ...
    async def search_maintenance_docs(
        ...
        Uses the document_embeddings table with document_type='equipment_health'.
        """
        ...
        sql += " ORDER BY rank_score DESC LIMIT $" + str(len(params) + 1)
        params.append(top_k)
